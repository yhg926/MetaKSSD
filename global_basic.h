...
/*compile mode for different alphabet: 1:nt reduction,2:AA*/
  ...
  #define IS_DIFF(X,Y) ( Objdist[(X)][(Y)] )
	...
	#define OBJ_ALPH 4
	#define OBJ_BITS 2
	#define BIN_SZ 65536 //16384 //50 //for test //16384
	#define IS_DIFF(X,Y) ( (X) != (Y) )
#endif
/**/
...
#define COMPONENT_SZ 6//or 7, default unit component dimension size = 16^(COMPONENT_SZ) or 1<<4*(COMPONENT_SZ)
#define CTX_SPC_USE_L 8 //ctx space occupy rate limit = 1/(1<<CTX_SPC_USE_L)  
