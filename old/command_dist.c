		...
		if(opt_val->refpath[0] != '\0'){
		  ...
		  if ((ref_mco_stat_fp = fopen(ref_db,"rb")) == NULL) err(errno,"mco stat file:%s",ref_db );
			...
			if( qryco_dstat_fpath != NULL ){
				...
				mco_cbd_co_dist(opt_val->refpath, opt_val->remaining_args[0], distdir, opt_val->p);
...
#pragma omp parallel for num_threads(p_fit_mem) reduction(+:all_ctx_ct) schedule(guided)
      ...
      for(int i = 0; i< p_fit_mem; i++ )
        free(CO[i]);
      free(CO);
/*****test code(may be removed after testing 20190328):combining all *.co.i into all.co.i *****/
			unsigned int *kmer_ct_componet = malloc(seqfile_stat->infile_num * sizeof(unsigned int));
			char cofname[PATHLEN]; struct stat cof_stat;
			int largest_kmer_ct = 0 ;
			for(int i = 0; i < component_num; i++){					
				for(int j =0; j< seqfile_stat->infile_num; j++){
					sprintf(cofname,"%s/%d.%d.co.%d", co_dir, j/BIN_SZ, j%BIN_SZ, i);
						stat(cofname,&cof_stat);
						kmer_ct_componet[j] = cof_stat.st_size/sizeof(unsigned int);
						if (kmer_ct_componet[j] > largest_kmer_ct ) 
							largest_kmer_ct = kmer_ct_componet[j];
				}

				char combined_cof[PATHLEN];
				sprintf(combined_cof,"%s/comb.co.%d",co_dir,i);
				FILE *com_cofp;
				if( (com_cofp = fopen(combined_cof,"wb")) == NULL) err(errno,"%s",combined_cof); 	
				fwrite(kmer_ct_componet,sizeof(unsigned int),seqfile_stat->infile_num,com_cofp);
				unsigned int *tmpco = malloc( sizeof(unsigned int) * largest_kmer_ct );
				FILE *cofp;
				for(int j =0; j< seqfile_stat->infile_num; j++){
					sprintf(cofname,"%s/%d.%d.co.%d", co_dir, j/BIN_SZ, j%BIN_SZ, i);
					if( (cofp = fopen(cofname,"rb")) == NULL) err(errno,"%s",cofname);
					
					fread(tmpco,sizeof(unsigned int),kmer_ct_componet[j],cofp);
					fwrite(tmpco,sizeof(unsigned int),kmer_ct_componet[j],com_cofp);
...
void mco_cbd_co_dist ( char *refmco_dname, char *qryco_dname, const char *distout_dir, int p_fit_mem)
{
  fprintf(logfp,"run mco_cbd_co_dist(), %d threads used\n",p_fit_mem);
  printf("run mco_cbd_co_dist(), %d threads used\n",p_fit_mem);
  FILE *refmco_dstat_fp, *qryco_dstat_fp;
  char *refmco_dstat_fpath = malloc(PATHLEN*sizeof(char));
  char *qryco_dstat_fpath = malloc(PATHLEN*sizeof(char));
  sprintf(refmco_dstat_fpath,"%s/%s",refmco_dname,mco_dstat);
...
#pragma omp parallel for  num_threads(p_fit_mem) schedule(guided)
    ...
    fprintf(logfp,"all %d co files' distance file initialized\n",co_dstat_readin.infile_num);
		...
		FILE *cbd_fcode_comp_fp;
//    unsigned int *kmer_ct_comp = malloc(sizeof(unsigned int) * co_dstat_readin.infile_num);
    ...
    for ( int j = 0; j < mco_dstat_readin.comp_num; j++ ) {
      ...
      gidobj_t** unit_arrmco_readin = read_unit_arrmco_file(mco_fcode);
			...
			sprintf(co_cbd_fcode,"%s/comb.co.%d",qryco_dname,j);
			...
			llong *fco_pos = malloc(sizeof(llong) * co_dstat_readin.infile_num);					
			fco_pos[0] = co_dstat_readin.infile_num;
			for(int m=1;m<co_dstat_readin.infile_num;m++)
				fco_pos[m] = fco_pos[m-1] + cbd_fcode_mem[m];
...
#pragma omp parallel for  num_threads(p_fit_mem) schedule(guided)
      for(int k=0; k< co_dstat_readin.infile_num; k++){
        ...
        int fd;
        if( ( (fd = open(dist_fcode,O_RDWR, 0600) ) == -1) )
         err(errno,"mco_cbd_co_dist()::distfile = %s[tid = %d]",dist_fcode,tid);

        ctx_obj_ct_t * ctx_obj_ct = mmap(NULL, binsz*(sizeof(ctx_obj_ct_t)), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
        if(ctx_obj_ct == MAP_FAILED) err(errno,"ctx_obj_ct mmap error");
				/*****new mco_cbd_co_dist_core block***/
				unsigned int ind,mcogid;
  			for(int n = 0; n < cbd_fcode_mem[k] ; n++){
    			...
    			if( unit_arrmco_readin[ind] != NULL ){
      			for(unsigned int p = 1; p< unit_arrmco_readin[ind][0] + 1; p++ ){
        			...
        			ctx_obj_ct[mcogid]++ ; //ctx_obj_ct_t [0] is ctx_ct, [1] is obj_c
     				}
    			}
  			}
				/********mco_cbd_co_dist_core block end*******/
       // ctx_obj_ct = mco_co_mmpdist_core(unit_arrmco_readin,co_fcode,ctx_obj_ct);
        if ( msync( ctx_obj_ct, binsz*(sizeof(ctx_obj_ct_t)), MS_SYNC ) < 0 )
