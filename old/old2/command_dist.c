...
#pragma omp parallel for num_threads(p_fit_mem) schedule(guided)
			for(int i = 0; i < component_num; i++){					
				//index file first element is length of first co file, cbd cofile contain only kmer(no index nor fsize)
				//get jth fsize by cof_index_in_cbdco[j] - cof_index_in_cbdco[j-1]
				size_t *cof_index_in_cbdco = malloc(seqfile_stat->infile_num  * sizeof(size_t) );					
				...
				for(int j = 0; j< seqfile_stat->infile_num; j++){
					...
					if(j==0) cof_index_in_cbdco[j] = cof_stat.st_size/sizeof(unsigned int);
					else cof_index_in_cbdco[j] = (size_t)cof_index_in_cbdco[j-1] + cof_stat.st_size/sizeof(unsigned int);  
					
					fread(tmpco,sizeof(unsigned int),cof_stat.st_size/sizeof(unsigned int),cofp);
					fwrite(tmpco,sizeof(unsigned int),cof_stat.st_size/sizeof(unsigned int),com_cofp);
				...
				fwrite(cof_index_in_cbdco, sizeof(size_t),seqfile_stat->infile_num,indexfp);
...
void mco_cbd_co_dist(dist_opt_val_t *opt_val_in)
{
  ...
  if (dist_bfp == -1) {
	...
	ftruncate(dist_bfp, disf_sz);
	close(dist_bfp);

	//mmap not consume memory? visit memory does?
  ...
  fco_pos[0] = co_dstat_readin.infile_num;
	...
	char mco_fcode[PATHLEN];char co_cbd_fcode[PATHLEN];
	for(int b=0;b<=num_mapping_distf;b++){			
    	...
    	for ( int j = 0; j < mco_dstat_readin.comp_num; j++ ) {
      	...
      	for(int m=1;m<co_dstat_readin.infile_num;m++)
        	fco_pos[m] = fco_pos[m-1] + cbd_fcode_mem[m-1]; // bug find: fco_pos[m] = fco_pos[m-1] + cbd_fcode_mem[m];

				printf("b=%d\ti=%d\tbeofore thread spwan ok\n",b,i);

#pragma omp parallel for  num_threads(p_fit_mem) schedule(guided)
      	for(int kind =0; kind < bnum_infile ; kind++){
					int k = b*num_cof_batch + kind; //offset in combined list
        	if(qry_ctx_ct_list[k]==0) continue;
		  	...
		  	printf("i=%d\tb=%d\toffset=%llu\tprim_addr=%p\toffset_addr=%p checked\n",i,b, (llong)b*num_cof_batch*mco_dstat_readin.infile_num,
					ctx_obj_ct, ctx_obj_ct + (llong)b*num_cof_batch*mco_dstat_readin.infile_num);	
