...
{
	struct arg_composite composite = {
		0,
	};
	...
	if (composite_opt.refdir[0] != '\0')
	{
		...
		{
			if (composite_opt.s >= 0 && composite_opt.s < 3 && composite_opt.remaining_args > 0)
				return abv_search(&composite_opt);
			else
				printf("\vUsage: metakssd composite -r <ref> -s <0|1|2> <query.abv>\n\v");
		}
		else
	...
	{
		if (composite_opt.num_remaining_args < 1)
			printf("\vUsage: kssd composite -d <query.abv>\n\v");
		else
			return read_abv(&composite_opt);
	}
	else
	{
		printf("\vUsage: metakssd composite -r <ref> < mode: -q | -i | -s >\n\v");
...
int read_abv(composite_opt_t *composite_opt)
{
	for (int i = 0; i < composite_opt->num_remaining_args; i++)
	{
		...
		FILE *abvfh;
		;
		if ((abvfh = fopen(composite_opt->remaining_args[i], "rb")) == NULL)
			err(errno, "read_abv():%s", composite_opt->remaining_args[i]);
...
int abv_search(composite_opt_t *composite_opt)
{
	...
	if ((namefh = fopen(abv_fpath, "r")) == NULL)
		err(errno, "abv_search():%s", abv_fpath);
	int abv_fn = 0;
	int size = 100;
	...
	if ((y_l2n_fh = fopen(abv_fpath, "rb")) == NULL)
		err(errno, "abv_search():%s", abv_fpath);
	...
	if ((abunMtx_idxfh = fopen(abv_fpath, "rb")) == NULL)
		err(errno, "abv_search():%s", abv_fpath);
	...
	if ((abunMtx_fh = fopen(abv_fpath, "rb")) == NULL)
		err(errno, "abv_search():%s", abv_fpath);
...
#define DFLT (-2)								 // set default measure, value should not in the range of l1norm l2norm cosine
	...
	if (composite_opt->s == 1)
		xny_pct = malloc(sizeof(xny_pct_t) * abv_fn);
	...
	for (int i = 0; i < composite_opt->num_remaining_args; i++)
	{
		...
		if ((abvfh = fopen(abv_fpath, "rb")) == NULL)
			err(errno, "abv_search():%s", abv_fpath);
		fread(tmp_abv, sizeof(binVec_t), abv_len, abvfh);

		for (int i = 0; i < abv_fn; i++)
			tmp_measure[i] = DFLT;
		if (composite_opt->s == 1)
			memset(xny_pct, 0, sizeof(xny_pct_t) * abv_fn);
...
int index_abv(composite_opt_t *composite_opt)
{
	...
	if (!abv_dir)
		err(errno, "index_abv(): %s does not exists! \n", abv_dpath);
	// just for ref_dstat.infile_num
	const char *ref_dstat_fpath = test_get_fullpath(composite_opt->refdir, co_dstat);
	if (ref_dstat_fpath == NULL)
		err(errno, "cannot find %s under %s ", co_dstat, composite_opt->refdir);
	FILE *ref_dstat_fp;
	if ((ref_dstat_fp = fopen(ref_dstat_fpath, "rb")) == NULL)
		err(errno, "index_abv():%s", ref_dstat_fpath);
	...
	for (int i = 0; i < ref_dstat.infile_num; i++)
		abunMtx_dynamic[i] = malloc(sizeof(binVec_t));
	...
	char *abv_fpath = malloc(PATHLEN);
	sprintf(abv_fpath, "%s/%s.%s", composite_opt->refdir, binVec_dirname, abunMtx_name_suffix);
	if ((namefh = fopen(abv_fpath, "w")) == NULL)
		err(errno, "index_abv():%s", abv_fpath);

	sprintf(abv_fpath, "%s/%s.%s", composite_opt->refdir, binVec_dirname, y_l2n_suffix);
	if ((y_l2n_fh = fopen(abv_fpath, "wb")) == NULL)
		err(errno, "index_abv():%s", abv_fpath);
	...
	sprintf(abv_fpath, "%s/%s.%s", composite_opt->refdir, binVec_dirname, abunMtx_suffix);
	if ((abunMtx_fh = fopen(abv_fpath, "wb")) == NULL)
		err(errno, "index_abv():%s", abv_fpath);
	for (int i = 0; i < ref_dstat.infile_num; i++)
	{
		if (abunMtx_count[i])
	...
	sprintf(abv_fpath, "%s/%s.%s", composite_opt->refdir, binVec_dirname, abunMtx_idx_suffix);
	if ((abunMtx_idxfh = fopen(abv_fpath, "wb")) == NULL)
		err(errno, "index_abv():%s", abv_fpath);
...
{ // by uniq kmer in a species
	...
	const char *ref_dstat_fpath = test_get_fullpath(composite_opt->refdir, co_dstat);
	if (ref_dstat_fpath == NULL)
		err(errno, "cannot find %s under %s ", co_dstat, ref_dstat_fpath);
	const char *qry_dstat_fpath = test_get_fullpath(composite_opt->qrydir, co_dstat);
	if (qry_dstat_fpath == NULL)
		err(errno, "cannot find %s under %s ", co_dstat, qry_dstat_fpath);
	...
	if (!qry_dstat.koc)
		err(errno, "get_species_abundance(): query has not abundance");
...
#define ED_PCTL (0.99) // end percentile
		...
		if (composite_opt->b)
		{
			if (strlen(composite_opt->outdir) < 3) // outdir not set, value:\"./\"
				...
				strcpy(tmpfname, composite_opt->outdir); // if -o set, abv will be in outdir, not binVec_dirname
			mkdir(tmpfname, 0777);
			sprintf(tmpfname, "%s/%s.%s", tmpfname, basename(qryname[qn]), binVec_suffix);
			if ((tmpfp = fopen(tmpfname, "wb")) == NULL)
				err(errno, "get_species_abundance():%s", tmpfname);
		}
		int num_pass = 0;
		float binVecsum = 0;
		for (int i = 0; i < ref_dstat.infile_num; i++)
		{
			...
			// average
			int sum = 0;
			for (int n = 1; n <= kmer_num; n++)
				sum += ref_abund[sort_ref[i]][n];
