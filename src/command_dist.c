...
int dist_dispatch(dist_opt_val_t *opt_val)
{
	...
	{ // mem_dispatch.has_arg = 1;
		...
		if (opt_val->refpath[0] != '\0')
		{
			...
			if (ref_db == NULL)
				err(errno, "need speficy the ref-sketch path for -r to run the query-ref search model");
			FILE *ref_mco_stat_fp;
			if ((ref_mco_stat_fp = fopen(ref_db, "rb")) == NULL)
				err(errno, "mco stat file:%s", ref_db);
			...
			if (qryco_dstat_fpath != NULL)
			{
				...
				if (co_qry_dstat.shuf_id != mco_ref_dstat.shuf_id)
					err(errno, "qry shuf_id: %d not match ref shuf_id: %d\ntry regenerate .co dir and feed -s the .shuf"
							   "file used to generated ref database",
						co_qry_dstat.shuf_id, mco_ref_dstat.shuf_id);
			...
			{ // test seq file format and search by .fas .fq
				...
				if (is_valid_fas_fq_in)
				{
					...
					err(errno, "please specify valid query genomes seq or .co file for database search");
			}
		} // database serach mode end

		// if only .co query
		else if (qryco_dstat_fpath != NULL)
		{ // convert .co folder opt_val->remaining_args[0]  to .mco
			...
			{ // 20190813: combined multiple batches qry/ to a single qry/
		...
		{ // if only query => test if is valid raw seq format
			infile_tab_t *infile_stat = dist_organize_infiles(opt_val);
			infile_fmt_count_t *qry_fmt_count;
			qry_fmt_count = infile_fmt_count(infile_stat);
			bool is_valid_fas_fq_in = (infile_stat->infile_num != 0) &&
			...
			{ // convert .fas .fq to co
				...
				err(errno, "not valid raw seq format");
					   ...
					   int *shuffled_seqfname_ind, const char *co_dir, int p_fit_mem)
{
	...
	{ // normal or abundance mode 20220707
...
#pragma omp parallel for num_threads(p_fit_mem) schedule(guided)
		for (int c = 0; c < component_num; c++)
		{
			...
			if ((com_cofp = fopen(tmpfname, "wb")) == NULL)
				err(errno, "%s", tmpfname);

			sprintf(tmpfname, "%s/%s.%d", co_dir, idx_prefix, c);
			if ((indexfp = fopen(tmpfname, "wb")) == NULL)
				err(errno, "%s", tmpfname);
	...
	FILE *coutfp;
	if ((coutfp = fopen(co_dstat_fullname, "wb")) == NULL)
		err(errno, "%s", co_dstat_fullname);
...
void run_stageII(const char *co_dstat_fpath, const char *dist_mco_dir, int p_fit_mem)
{
	...
	if ((co_stat_fp = fopen(co_dstat_fpath, "rb")) == NULL)
		err(errno, "run_stageII(():%s", co_dstat_fpath);
	...
	if ((mco_stat_fp = fopen(mco_dstat_fpath, "wb")) == NULL)
		err(errno, "run_stageII(():%s", mco_dstat_fpath);
...
#pragma omp parallel for num_threads(p_fit_mem) schedule(guided)
		for (int k = 0; k < co_dstat_readin.infile_num; k++)
		{
			...
			FILE *distfp;
			if ((distfp = fopen(dist_fcode, "wb")) == NULL)
				err(errno, "mco_co_dist()::%s", dist_fcode);
...
#pragma omp parallel for num_threads(p_fit_mem) schedule(guided)
			for (int k = 0; k < co_dstat_readin.infile_num; k++)
			{
				...
				if (ctx_obj_ct == MAP_FAILED)
					err(errno, "ctx_obj_ct mmap error");
	...
	FILE *distfp;
	if ((distfp = fopen(distf, "a")) == NULL)
		err(errno, "mco_co_dist():%s", distf);
	for (int i = 0; i <= ref_bin_num; i++)
	{
		for (int k = 0; k < co_dstat_readin.infile_num; k++)
		{
...
void mco_cbd_co_dist(dist_opt_val_t *opt_val_in)
{
	...
	if (dist_bfp == -1)
	{
		...
		if (dist_bfp == -1)
			err(errno, "mco_cbd_co_dist()::%s", onedist); // reset name after test
	}
	else
	{
		errno = EEXIST;
	...
	if (ftruncate(dist_bfp, disf_sz) == -1)
		err(errno, "mco_cbd_co_dist()::ftruncate");
	close(dist_bfp);
	...
	if (comp_sz % page_sz != 0)
		err(errno, "comp_sz %ld is not multiple of page_sz %d ", comp_sz, page_sz);
...
void mco_cbdco_nobin_dist(dist_opt_val_t *opt_val_in) // currently used version(20220802)
{
	...
	if (dist_bfp == -1)
	{
		...
		dist_bfp = open(onedist, O_RDWR | O_CREAT, 0600);
		if (dist_bfp == -1)
			err(errno, " mco_cbdco_nobin_dist()::%s", onedist); // reset name after test
	}
	else
		err(EEXIST, " mco_cbdco_nobin_dist():%s", onedist);
...
#define MCOMM_SZ 442317172
	...
	{ // use previous share_k_ct for distance printing
		dist_print_nobin(distout_dir, ref_seq_num, qry_seq_num, ref_ctx_ct_list, qry_ctx_ct_list, num_cof_batch, mcofname, cofname, opt_val_in);
		return;
	}
	size_t unitsz_distf_mapped = (size_t)num_cof_batch * mco_dstat_readin.infile_num * sizeof(ctx_obj_ct_t);
	...
	if (ftruncate(dist_bfp, disf_sz) == -1)
		err(errno, "mco_cbdco_nobin_dist()::ftruncate");
	close(dist_bfp);
	dist_bfp = open(onedist, O_RDWR, 0600);
	if (dist_bfp == -1)
		err(errno, "mco_cbdco_nobin_dist()::%s", onedist);
	ctx_obj_ct_t *ctx_obj_ct = mmap(NULL, disf_sz, PROT_READ | PROT_WRITE, MAP_SHARED, dist_bfp, 0);
	if (ctx_obj_ct == MAP_FAILED)
		err(errno, "ctx_obj_ct mmap error");
	close(dist_bfp);
...
void mco_cbd_koc_compatible_dist(dist_opt_val_t *opt_val_in) //  latest version before 20220802
{
	...
	if (dist_bfp == -1)
	{
		...
		err(EEXIST, "mco_cbd_koc_compatible_dist:%s", onedist);
...
#define MCOMM_SZ 442317172
	...
	if (ftruncate(dist_bfp, disf_sz) == -1)
		err(errno, "mco_cbd_koc_compatible_dist()::ftruncate");
	close(dist_bfp);
	dist_bfp = open(onedist, O_RDWR, 0600);
	if (dist_bfp == -1)
		err(errno, "mco_cbd_koc_compatible_dist()::%s", onedist);
	ctx_obj_ct_t *ctx_obj_ct = mmap(NULL, disf_sz, PROT_READ | PROT_WRITE, MAP_SHARED, dist_bfp, 0);
	if (ctx_obj_ct == MAP_FAILED)
		err(errno, "ctx_obj_ct mmap error");
	close(dist_bfp);
	...
	for (int b = 0; b <= num_mapping_distf; b++)
	{
		...
		for (int j = 0; j < mco_dstat_readin.comp_num; j++)
		{
			...
			struct stat s;
			if ((mcofp = fopen(mco_fcode, "rb")) == NULL)
				err(errno, "mco_cbd_koc_compatible_dist()::%s", mco_fcode);
			stat(mco_fcode, &s);
			if (s.st_size > sizeof(gidobj_t) * MCOMM_SZ)
				mco_mem = (gidobj_t *)realloc(mco_mem, s.st_size);
			fread(mco_mem, sizeof(gidobj_t), s.st_size / sizeof(gidobj_t), mcofp);
			fclose(mcofp);
			/******mco fread block >*****/
					  ...
					  unsigned int *qry_ctx_ct_list, char (*refname)[PATHLEN], char (*qryfname)[PATHLEN], FILE *dout_fp)
{
	...
	for (llong i = 0; i < s.st_size / sizeof(ctx_obj_ct_t); i++)
	{
		...
		double rs = P_K_in_X_XnY * P_K_in_Y_XnY * (X_XnY_size + Y_XnY_size) / (P_K_in_X_XnY + P_K_in_Y_XnY - 2 * P_K_in_X_XnY * P_K_in_Y_XnY);
						  ...
						  unsigned int *qry_ctx_ct_list, int num_cof_batch, char (*refname)[PATHLEN], char (*qryfname)[PATHLEN])
{
	...
	if (distfp == NULL)
		err(errno, "dist_print_nobin():%s", distf);
		...
		for (int qid = 0; qid < bnum_infile; qid++)
		{
			...
			for (int rid = 0; rid < ref_num; rid++)
				if (ctx_obj_ct[((llong)b * num_cof_batch + qid) * ref_num + rid].shared_k_ct > 0)
					abundence_rowsum += (double)ctx_obj_ct[((llong)b * num_cof_batch + qid) * ref_num + rid].shared_koc_ct / (double)ctx_obj_ct[((llong)b * num_cof_batch + qid) * ref_num + rid].shared_k_ct;
			...
			for (int rid = 0; rid < ref_num; rid++)
			{
				...
				double abundence_pct = (double)ctx_obj_ct[((llong)b * num_cof_batch + qid) * ref_num + rid].shared_koc_ct / XnY_size; // / abundence_rowsum ;
...
{ // 1. matrix or not? 2. only subset or full columns 3. maximal distance/num of referece reported
	...
	if (opt_val->shared_kmerpath[0] != '\0')
		strcpy(full_distfcode, opt_val->shared_kmerpath);
	else
		sprintf(full_distfcode, "%s/sharedk_ct.dat", distout_dir);
	int fd;
	struct stat s;
...
#define NREF 1024
	...
	for (int b = 0; b <= num_mapping_distf; b++)
	{
		...
		for (int qid = 0; qid < bnum_infile; qid++)
		{
			...
			if (N_max)
			{ // pick N references with largest jaccared
				for (int i = 0; i < N_max; i++)
					bestNref[i] = (Nref_stuct){0, -1};
				for (int rid = 0; rid < ref_num; rid++)
				{
					unsigned int X_size = ref_ctx_ct_list[rid];
					unsigned int XnY_size = ctx_obj_ct[offset + rid];
					double metric = outfield.metric == Ctm ? (double)XnY_size / (X_size < outfield.Y_size ? X_size : outfield.Y_size) : // sort by Ctm only id metric is Ctm
					...
					for (int i = N_max - 1; i >= 0; i--)
					{
						...
						{ // for larger selection, otherwise use < and differnt bestNref[NREF] initialization values
...
#pragma omp parallel for num_threads(p_fit_mem) schedule(guided)
				for (int i = 0; i < N_max; i++)
				{
					if (bestNref[i].rid != -1) // fix bug :20200504
						...
						prt_buf[i].len = 0; // 20200504: filtered out when fwrited to ./disntance.out in next step
				}
				for (int i = 0; i < N_max; i++)
					if (prt_buf[i].len > 1)
						fwrite(prt_buf[i].line, prt_buf[i].len, 1, distfp);
				;
			}
			else
			{
#pragma omp parallel for num_threads(p_fit_mem) schedule(guided)
				...
				for (int rid = 0; rid < ref_num; rid++)
					if (prt_buf[rid].len > 1)
						fwrite(prt_buf[rid].line, prt_buf[rid].len, 1, distfp);
				;
			} // else
		} // qry loop in one batch
		munmap(ctx_obj_ct + (size_t)b * num_cof_batch * ref_num, maplength);
...
static inline void output_ctrl(unsigned int X_size, unsigned int XnY_size, print_ctrl_t *outfield, char *rname, prt_line_t *linebuf)
{
	...
	if (outfield->correction)
	{
		...
		rs = P_K_in_X_XnY * P_K_in_Y_XnY * (X_XnY_size + Y_XnY_size) / (P_K_in_X_XnY + P_K_in_Y_XnY - 2 * P_K_in_X_XnY * P_K_in_Y_XnY);
	}
...
const char *combine_queries(dist_opt_val_t *opt_val)
{
	...
	if (opt_val->abundance)
	{
		err(errno, "combine_queries(): abundance model not supported yet");
	}
	FILE *co_stat_fp;
	const char *qryco_dstat_fpath = NULL;
	qryco_dstat_fpath = test_get_fullpath(opt_val->remaining_args[0], co_dstat);
	...
	FILE *kmerct_list_fp;
	if ((kmerct_list_fp = fopen(one_kmerct_list_name, "wb")) == NULL)
	{
		err(errno, "%s", one_kmerct_list_name);
	}
	...
	if ((infilename_name_fp = fopen(one_infilename_name, "wb")) == NULL)
	{
		err(errno, "%s", one_infilename_name);
	}
...
#pragma omp parallel for num_threads(p_fit_mem) schedule(guided)
	for (int c = 0; c < co_dstat_one.comp_num; c++)
	{
		...
		if ((com_cofp[c] = fopen(combined_cof, "wb")) == NULL)
			err(errno, "%s", combined_cof);
		...
		FILE *com_cofp_it;
		if ((com_cofp_it = fopen(combined_cof, "rb")) == NULL)
			err(errno, "%s", combined_cof);
		...
		if ((indexfp[c] = fopen(indexfname, "wb")) == NULL)
			err(errno, "%s", indexfname);
		...
		FILE *com_indexfp_it;
		if ((com_indexfp_it = fopen(indexfname, "rb")) == NULL)
			err(errno, "%s", indexfname);
...
#pragma omp parallel for num_threads(p_fit_mem) schedule(guided)
		for (int c = 0; c < co_dstat_one.comp_num; c++)
		{
			...
			if ((com_cofp_it = fopen(combined_cof_it, "rb")) == NULL)
				err(errno, "%s", combined_cof_it);
			...
			FILE *indexfp_it;
			if ((indexfp_it = fopen(indexfname_it, "rb")) == NULL)
				err(errno, "%s", indexfname_it);
			fread(tmpindex, file_stat.st_size, 1, indexfp_it);
			fclose(indexfp_it);

			int tmp_infile_num = file_stat.st_size / sizeof(size_t);
			for (int i = 1; i < tmp_infile_num; i++)
			{
				tmpindex[i] += index_offset[c];
			}
...
#pragma omp parallel for num_threads(p_fit_mem) schedule(guided)
	...
	if ((one_co_stat_fp = fopen(one_stat_name, "wb")) == NULL)
	{
		err(errno, "%s", one_stat_name);
	};
	...
	if ((kmerct_list_fp = fopen(one_kmerct_list_name, "rb")) == NULL)
	{
		err(errno, "%s", one_kmerct_list_name);
	}
	...
	if ((infilename_name_fp = fopen(one_infilename_name, "rb")) == NULL)
	{
		err(errno, "%s", one_infilename_name);
	}
